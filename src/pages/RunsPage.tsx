...
function formatRunLabel(label: string, stripModel = false): string {
  ...
  // New convention: "dd/MM/yy - X kw - llm_name" → strip LLM suffix for runs page
  if (stripModel) {
    return label.replace(/\s*-\s*(?!.*\bkw\b)[^-]+$/, '').trim();
  }
  return label;
}
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-sm text-zinc-900">{formatRunLabel(run.run_label, true)}</span>
